...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    peer_cert peer peer0.org1.node.connection org1
    peer_cert admin Admin@org1.node.connection org1

    infoln "Creating Org2 Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.org2.node.connection org2
    peer_cert admin Admin@org2.node.connection org2

    infoln "Creating Orderer Org Identities"
    #function_name cert-type   CN   
    orderer_cert orderer orderer.node.connection
    orderer_cert admin Admin@node.connection
...
function networkDown() {
  ...
  COMPOSE_FILE_BASE=compose-bft-node-connection.yaml
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.node.connection docker_peer0.org1.node.connection docker_peer0.org2.node.connection
...
COMPOSE_FILE_BASE=compose-node-connection.yaml
